...
namespace EcommerceStore.Bff.Compras.Controllers
{
    public class PedidoController : MainController
    {
        ...
        [HttpGet("compras/pedido/lista-pedidos")]
        public async Task<IActionResult> ListaTodosOsPedidos([FromQuery] int pageSize, [FromQuery] int page)
        {
            var pedidos = await _pedidoService.ObterTodosOsPedidos(pageSize, page);

            return pedidos == null ? NotFound() : CustomResponse(pedidos);
        }


        [HttpGet("compras/pedido/voucher/{codigo}")]
        ...
        [HttpPost("compras/pedido/voucher")]
        public async Task<IActionResult> AdicionarVoucher(CriarVoucherDTO criarVoucherDTO)
        {
            return CustomResponse(await _pedidoService.AdicionarVoucher(criarVoucherDTO));
        }

        [HttpGet("compras/pedido/lista-vouchers")]
        public async Task<IActionResult> ObterVouchers([FromQuery] int pageSize, [FromQuery] int page)
        {
            var vouchers = await _pedidoService.ObterVouchers(pageSize, page);

            return CustomResponse(vouchers);
        }
